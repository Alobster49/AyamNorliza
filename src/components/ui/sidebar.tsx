      ...
      <Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
        ...
        >
          <SheetHeader className="sr-only">
            <SheetTitle>{t("sidebarLabel")}</SheetTitle>
...
function SidebarInset({ className, ...props }: React.ComponentProps<"main">) {
  return (
    <main
      ...
      className={cn(
        "relative flex w-full flex-1 flex-col bg-background md:peer-data-[variant=inset]:m-2 md:peer-data-[variant=inset]:ml-0 md:peer-data-[variant=inset]:mr-0 md:peer-data-[variant=inset]:rounded-2xl md:peer-data-[variant=inset]:shadow-sm",
        className
      )}
      {...props}
    />
    ...
    asChild && React.isValidElement(children) ? (
      React.cloneElement(
        children as React.ReactElement<Record<string, unknown>>,
        {
          ...
          className: cn(
            mergedClassName,
            (children as React.ReactElement<Record<string, unknown>>).props.className as string | undefined
          ),
        }
      )
    ) : (
