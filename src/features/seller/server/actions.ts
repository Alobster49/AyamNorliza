...
export async function requireSellerRole(orgSlug: string): Promise<boolean> {
  ...
  const { data: member } = await supabase
    ...
    .select("role")
    .eq("organization_id", await getOrganizationId(orgSlug))
    .single();
...
export async function createCategory(orgId: string, input: Omit<CategoryInsert, "organization_id">) {
  const supabase = await createClient();
...
export async function createProduct(orgId: string, input: Omit<ProductInsert, "organization_id">) {
  const supabase = await createClient();
  const { data, error } = await supabase
    .from("products")
    .insert({ ...input, organization_id: orgId })
...
// Product Variants
// ---------------------------------------------------------------------------
export async function createVariant(orgId: string, input: Omit<ProductVariantInsert, "organization_id">) {
  const supabase = await createClient();
  const { data, error } = await supabase
...
export async function updateVariant(id: string, input: ProductVariantUpdate) {
  const supabase = await createClient();
  const { data, error } = await supabase
    .from("product_variants")
    .update(input)
...
export async function createCustomer(orgId: string, input: Omit<CustomerInsert, "organization_id" | "created_by">) {
  const supabase = await createClient();
  ...
  if (error) throw new Error(error.message);
  revalidatePath(`/[organizationSlug]/customers`);
  return data;
}
