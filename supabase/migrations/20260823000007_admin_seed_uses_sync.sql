-- 20260823000007_admin_seed_uses_sync.sql
-- admin_seed_demo_data used to relink existing buyer accounts with a
-- hand-rolled insert-then-blanket-update (insert a customers row keyed on
-- the buyer's own id, then unconditionally point customer_id at that same
-- id for every buyer in the org) that bypassed the phone-match "oldest
-- unclaimed wins" / no-stealing rules the signup trigger now enforces via
-- public.link_or_create_customer_for_buyer (20260823000006). Route the seed
-- through the same shared function so admin-seeded orgs behave identically
-- to organically-signed-up ones.
