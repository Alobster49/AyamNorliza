-- 20260827000003_seed_categories_and_history.sql
-- Demo seed catches up with the current order flow and gets real history.
--
-- What changed since the last seed definition (20260826000001):
--   * price-at-confirm (20260826000001): every line that survives confirm
--     carries price_per_kg. The old seed left confirmed/ready lines unpriced,
--     which the app can no longer produce.
--   * driver door weights close the order (20260826000002 + 20260827000002):
--     a driver-settled stop ends 'closed' with final weights, final pieces and
--     closed_at -- not 'delivered'. 'delivered' now only comes from the office
--     "complete run" sweep, and those orders sit in the settlement queue.
--
-- So this re-definition:
--   * splits the catalog into two categories: 'Ayam Segar' (whole birds) and
--     'Part Ayam' (cuts) -- matching how the shop actually sells.
--   * prices every confirmed/ready/closed line with realistic RM/kg deal
--     prices; pending and cancelled lines stay unpriced.
--   * turns yesterday's run B into the current shapes: two stops driver-settled
--     ('closed', final weights + logs), one office-swept ('delivered', no
--     final weight) so the settlement queue has something to settle.
--   * adds three weeks of settled history (21 completed runs, 49 closed
--     orders) so dashboards, reports and invoices open on real-looking data.
--     Everything is derived deterministically from the day offset, so the
--     seed stays idempotent and the RLS test can assert exact counts
--     (products 13, customers 11, orders 64, runs 23).
...
begin
  ...
  -- A (2 loaded), and on run B: 2 driver-settled (closed) + 1 office-swept
  -- (delivered, unsettled), 1 cancelled = 15.
  ...
  -- Tasks: open for 805/806, done for the weighed/ready/run-B orders.
