...
- Console accounts after seed: one per role the app gates on -- `owner@gmail.com` (`owner`), `admin@gmail.com` (`org_admin`), `seller@gmail.com` (`seller`), `warehouse@gmail.com` (`inventory`), `driver1@gmail.com` and `driver2@gmail.com` (`driver`) -- all password `password123` (accepted risk, committed to repo). Seeded runs are handed to the drivers round-robin so the driver deck is not empty.
...
3. Seed: success message with counts; products page shows 13 products **with images rendering** (this catches any image-path typo); orders kanban, tasks, dispatch, loading, runs pages all show data; login as `owner@gmail.com` / `password123` works, and the login page's "Dev: pick an account" dialog fills any seeded account.
